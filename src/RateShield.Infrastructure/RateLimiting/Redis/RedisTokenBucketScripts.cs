...
public static class RedisTokenBucketScripts
{
    ...
    public const string EvaluateTokenBucket = """
        ...
        local newMs = (tonumber(redisTime[1]) * 1000) + math.floor(tonumber(redisTime[2]) / 10000)
        ...
        if elapsedMs >= refillPeriodMs then
           local periodsElapsed = math.flooor(elapsedMs / refillPeriodMs)
        ...
        if tokens >= requestCost then
           ...
           local tokensNeeded = requestCost - tokiens
